...
while True:
  ...
  bbox = [480, 230, 10, 10] # Example bounding box (x, y, width, height)
  x, y, w, h = bbox
  average_depth = calculate_bbox_depth(disparity, bbox)
  print(f"Average depth in bounding box {bbox}: {average_depth:.2f} meters")
  print(f"Disparity: {disparity[y-h:y+h, x-w:x+w]}")
  print(f"Processing time: {(time.time() - start_time)*1000} ms")
  ...
  cv2.imshow('Depth color map', cv2.hconcat([image_left, colormap_image]))
